...
def generate_keywords_with_openai(base_keyword, model_shortname, config_file):
    ...
    """
    print(
        f"Generating keywords based on '{base_keyword}' using model '{model_shortname}'..."
    )
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Error: Model configuration file '{config_file}' not found."
        )
    ...
    if not model_config:
        raise ValueError(
            f"Error: Model shortname '{model_shortname}' not found in '{config_file}'."
        )
    ...
    if openai_api_key_config == "${OPENAI_API_KEY}":
        ...
        raise ValueError(
            f"Error: 'openai_api_key' not specified for model '{model_shortname}' "
            f"in '{config_file}'."
        )
    ...
    if not openai_api_base or not openai_model:
        raise ValueError(
            f"Error: 'openai_api_base' or 'openai_model' missing for model "
    ...
    # --- 4. Define Prompts ---
    system_prompt = (
        "You are an AI assistant helping with research. Generate related keywords "
        "based on the user's input."
    )
    user_prompt = (
        f"Given the base keyword '{base_keyword}', generate 5 related research "
    ...
    try:
        ...
        # --- 6. Parse Response ---
        generated_keywords = [
            kw.strip() for kw in generated_text.strip().split("\n") if kw.strip()
        ]
...
def check_relevance_with_openai(
    abstract: str, keyword: str, model_shortname: str, config_file: str
) -> bool:
    """
    ...
    print(
        f"Checking relevance of abstract against keyword '{keyword}' using model '{model_shortname}'..."
    )
    ...
    try:
        ...
        print(
            f"Error: Model configuration file '{config_file}' not found during relevance check."
        )
        return False
    except yaml.YAMLError as e:
    ...
    if not model_config:
        print(
            f"Error: Model shortname '{model_shortname}' not found in '{config_file}' for relevance check."
        )
        return False
    ...
    if openai_api_key_config == "${OPENAI_API_KEY}":
        ...
        if not openai_api_key:
            print(
                "Error: OpenAI API key ('OPENAI-API-KEY' or 'OPENAI_API_KEY' env var) not set for relevance check."
            )
            return False
    elif openai_api_key_config:
        openai_api_key = openai_api_key_config
        ...
        print(
            f"Error: 'openai_api_key' not specified for model '{model_shortname}' for relevance check."
        )
        return False
    ...
    if not openai_api_base or not openai_model:
        print(
            f"Error: 'openai_api_base' or 'openai_model' missing for model '{model_shortname}'."
        )
        return False
    ...
    truncated_abstract = abstract[:max_abstract_chars] + (
        "..." if len(abstract) > max_abstract_chars else ""
    )

    user_prompt = (
        f"Analyze the following abstract based on the keyword '{keyword}'. "
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download academic papers from Semantic Scholar."
    )
    ...
    group.add_argument(
        "--generate-keywords",
        type=str,
        help="A base keyword to generate related keywords using OpenAI.",
    )
    group.add_argument(
        "--keywords",
        type=str,
        help='Comma-separated keywords to search for (e.g., "machine learning,neural networks,deep learning").',
    )

    parser.add_argument(
        "--model",
        type=str,
        help="Shortname of the OpenAI model (from model_servers.yaml) to use for keyword generation. Required if --generate-keywords is used.",
    )
    parser.add_argument(
        "--relevance-model",
        type=str,
        default=None,
        help="Shortname of the OpenAI model (from model_servers.yaml) to use for relevance checking. If not provided, relevance check is skipped.",
    )
    parser.add_argument(
        "--skip-relevance-check",
        action="store_true",
        default=False,
        help="Skip the abstract relevance check before downloading PDFs.",
    )
    parser.add_argument(
        "--max-relevant-papers",
        type=int,
    ...
    if ss_api_key:
        if os.environ.get("SS-API-KEY"):
            print(
                "Using Semantic Scholar API key from environment variable 'SS-API-KEY'."
            )
        elif os.environ.get("SEMANTIC_SCHOLAR_API_KEY"):
            print(
                "Using Semantic Scholar API key from environment variable 'SEMANTIC_SCHOLAR_API_KEY'."
            )
        else:
            print("Using Semantic Scholar API key from command line argument.")
    else:
        print(
            "Warning: No Semantic Scholar API key provided (via env vars SS-API-KEY/SEMANTIC_SCHOLAR_API_KEY or --ss-api-key). Rate limits may be stricter."
        )
    ...
    if args.keywords_file:
        ...
        try:
            keywords = generate_keywords_with_openai(
                args.generate_keywords, args.model, args.config
            )
            if not keywords:
    ...
    for keyword in keywords:
        ...
        if os.path.exists(processed_papers_file):
            try:
                ...
                print(
                    f"Loaded {len(processed_papers)} processed paper IDs from {processed_papers_file}"
                )
            except Exception as e:
                print(
                    f"Warning: Could not read {processed_papers_file}. Starting fresh for this keyword. Error: {e}"
                )
                processed_papers = set()
        ...
        print(
            f"Attempting to fetch up to {total_papers_to_fetch} papers for '{keyword}'..."
        )
        for offset in range(0, total_papers_to_fetch, fetch_limit):
            ...
            if data and "data" in data:
                ...
                print(
                    f"Retrieved {len(papers)} papers for '{keyword}' at offset {offset}."
                )
                # Check if 'next' field indicates more results, or if fewer than limit returned
                if len(papers) < fetch_limit or not data.get("next"):
                    print(
                        "Reached end of results or fetched less than limit, stopping fetch loop for this keyword."
                    )
                    break
            else:
                print(
                    f"No valid data returned or error occurred for '{keyword}' at offset {offset}. Stopping fetch loop for this keyword."
                )
        ...
        for paper in all_papers:
            ...
            if (
                args.max_relevant_papers
                and relevant_papers_downloaded >= args.max_relevant_papers
            ):
                print(
                    f"Reached maximum of {args.max_relevant_papers} relevant papers for keyword '{keyword}'. Stopping processing."
                )
                break
            ...
            if not args.skip_relevance_check and args.relevance_model and abstract:
                try:
                    is_relevant = check_relevance_with_openai(
                        abstract, keyword, args.relevance_model, args.config
                    )
            ...
            if not is_relevant:
                print(
                    f"Skipping paper ID {paper_id} ('{title}') due to relevance check."
                )
                ...
                try:
                    ...
                    print(
                        f"Error writing skipped-relevance paper ID to processed file {processed_papers_file}: {e}"
                    )
                ...
                if not os.path.exists(save_path):
                    ...
                    if download_pdf(pdf_url, save_path):
                        if os.path.exists(save_path):  # Verify download succeeded
                            ...
                            print(
                                f"Download reported success but file not found: {save_path}"
                            )
                    else:
            ...
            try:
                ...
                print(
                    f"Error writing to processed papers file {processed_papers_file}: {e}"
                )
